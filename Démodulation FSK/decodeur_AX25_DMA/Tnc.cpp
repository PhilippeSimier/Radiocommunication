...
void Tnc::begin(int p_led) {    
    ...
    configureI2s();
    xTaskCreatePinnedToCore(Tnc::marshall, "adc_i2s", 50000, &led, 3, &TaskHandle_Dac, 1); // creation de la tache 
}

void Tnc::configureI2s() {
    i2s_config_t i2s_config =
  {
    .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX | I2S_MODE_TX | I2S_MODE_DAC_BUILT_IN | I2S_MODE_ADC_BUILT_IN),  // I2S receive mode with ADC and dac
    .sample_rate = SAMPLING_FREQUENCY,                                             // fréquence d'échantillonage
    .bits_per_sample = I2S_BITS_PER_SAMPLE_16BIT,                                 // 16 bit I2S
    .channel_format = I2S_CHANNEL_FMT_ONLY_RIGHT,                                 // seulement le canal droit
    .communication_format = I2S_COMM_FORMAT_I2S_MSB,                              // MSB 1st
    .intr_alloc_flags = 0,                                                        // none
    .dma_buf_count = 4,                                                           // nombre de buffers DMA
    .dma_buf_len = NUM_SAMPLES,                                                   // nombre d'échantillons
    .use_apll = 0, // no Audio PLL
  };
  adc1_config_channel_atten(PIN_ADC, ADC_ATTEN_11db);                         // tension pleine échelle Aref 3.3V
adc1_config_width(ADC_WIDTH_12Bit); // adc 12 bits
  i2s_driver_install(I2S_NUM_0, &i2s_config, 0, NULL);                            // configuration i2s sur le canal 0 (pas possible ADC/DAC sur le canal 1)

  i2s_set_adc_mode(ADC_UNIT_1, PIN_ADC);                                      //configuration de la broche d'entrée adc
  SET_PERI_REG_MASK(SYSCON_SARADC_CTRL2_REG, SYSCON_SARADC_SAR1_INV);             //inversion tension d'entrée sur l'adc pour avoir 3.3V=4095
i2s_set_dac_mode(I2S_DAC_CHANNEL_RIGHT_EN); //configuration de la broche de sortie du DAC

i2s_adc_enable(I2S_NUM_0); //mise en route du dma et i2s pour l'ADC et DAC 
    ...
    while (1) {
        ...
        digitalWrite(led,digitalRead(led)^1);
                for (int i = 0; i < NUM_SAMPLES; i++) {
                    decode((int16_t) i2s_read_buff[i]>>4);
                }
    }
}

void Tnc::marshall(void * parametres) {
    anchor->dma();
}
...
void Tnc::decode(int16_t rawadc)
